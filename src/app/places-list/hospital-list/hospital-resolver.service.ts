...
export class HospitalsResolverService implements Resolve<Promise<Hospital[]>> {
  ...
  async resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<Hospital[]> {
    const hospitals = this.hospitalService.getHospitals();
    const latlng: Location = await this.dataStorageService.getLocation();
    if (hospitals.length === 0) {
      return this.dataStorageService.fetchHospitals(latlng.latitude, latlng.longitude).toPromise();
    } else {
      return hospitals;
    }
  }

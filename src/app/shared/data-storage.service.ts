...
export class DataStorageService {
  ...
  fetchHospitals(latitude = 53.0, longitude = 0) {
    return this.http
      .get<Hospital[]>(
        `${this.url}hospitals/?latitude=${latitude}&longitude=${longitude}`
      )
      .pipe(
        map((hospitals: any) => {
          return hospitals.results.map((hospital: any) => {
            return new Hospital(
              ...
              hospital.longitude,
              hospital.distance,
              hospital.has_vaccine,
              hospital.infected_number
            );
          });
        }),
        tap((hospitals) => {
  ...
  fetchLaboratories(latitude = 53.0, longitude = 0) {
    return this.http
      .get<Laboratory[]>(
        `${this.url}laboratories/?latitude=${latitude}&longitude=${longitude}`
      )
      .pipe(
        map((laboratory: any) => {
  ...
  getReversedGeolocation(latitude: number, longitude: number) {
    const url = `${this.url}reverse-geocoding/?latlng=${latitude},${longitude}`;
  ...
  getGeocoding(address: string) {
    const url = `${this.url}geocoding/?address=${address}`;
    return this.http.get<Location>(url).pipe(
      map((location: any) => {
        if (location) {
          const geolocation: Location = {
